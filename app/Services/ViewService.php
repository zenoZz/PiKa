...
class ViewService
{
    ...
    public static function renderSidebar($menus, $route)
    {
        $user = Auth::user();
        if ( ! $user) {
    ...
    public static function makeSidebar(array $menus, $active)
    {
        $sidebar = "";

        foreach ($menus as $menu) {
            if ($menu['hide'] == Menu::MENU_HIDE_NO) {
                if (isset($menu['_child']) && !empty($menu['_child'])) {
                        ...
                        self::makeSidebar($menu['_child'], $active) .
                            '</ul>
                        </li>';
                } else {
                    if (in_array($menu['route'], $active)) {
