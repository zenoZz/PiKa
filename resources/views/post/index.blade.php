...
@section('content')
    @if(isset($articles) && !empty($articles))
        ...
        <article id="{$val['id']}" class="post tag-laravel tag-laravel-5-1 tag-xin-ban-ben-fa-bu" style="margin-bottom:20px;">
            ...
            @endif
            <div class="post-head">
                <h1 class="post-title"><a href="/post/article/{{ $article->getKey() }}">{{$article->title}}</a></h1>
                <div class="post-meta">
                    <span class="author">作者：<a href="javascript:void (0)" style="color: #f4645f">{{$article->author}}</a></span> &bull;
